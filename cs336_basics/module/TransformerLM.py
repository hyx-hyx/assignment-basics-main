...
class TransformerLM(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int,
        ...
        for i in range(0,num_layers):
            ...
            transformer_block=self.TransformerBlock(d_model,num_heads,d_ff,context_length,theta,weights)
    ...
    def forward(self,x: torch.Tensor):
        ...
        for i in range(0,len(self.transformer_blocks)):
            y=self.transformer_blocks[i].forward(y)
        ...
        y=linear.forward(rmsnorm.forward(y))
        return softmax(y,y.ndim-1)
